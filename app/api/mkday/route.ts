...
export const GET = async (request:  NextRequest) => {
  ...
  try{ // esentially just making a homemade UPSERT here
    ...
    const values:string[] = ['uidtest', day, ship]; // we probably want some overlap occuring here,
    //i just need to figure out how to make this pull anything lolge
    
    const query = "DELETE FROM days WHERE uid='"+values[0]+"' and day='"+values[1]+"'"; // this properly deletes & covers overlap
