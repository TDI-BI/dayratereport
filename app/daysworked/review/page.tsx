...
const Content = ({prev}: { prev: number }) => {
    ...
    const submit = async () => {
        ...
        if (bleh.status === 200) {
            ...
            const stat = await bleh.json();
            setUmsg(`error: ${stat.error}`);
        }
    ...
    return (
        <main className="flex min-h-screen flex-col items-center px-5 py-5">
            <div
                className={`text-center font-semibold text-lg py-[10px] ${saving ? 'opacity-0' : 'opacity-100'} transition-all ease-in-out duration-300`}>
                CONFIRM YOUR REPORT
            </div>
            ...
            <div
                className={`inline-flex flex-row gap-[5px] ${saving ? 'opacity-0' : 'opacity-100'} transition-all ease-in-out duration-300`}>
                <button
